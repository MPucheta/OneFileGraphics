...
async function main () {
  ...
  const obj1 = allocMemoryForObject();
  const light = allocMemoryForObject();

  function setupFPSInputControls () {
    function subscribeToMouseMovement () {
      function updateCameraPosition (e) {
        // tricking mouse delta movement into radians to rotate camera
        // clipspace inverted, so we -1 it
        CONTROLS.cameraYaw -= e.movementX * CONTROLS.aimSensitivity;
        CONTROLS.cameraPitch -= e.movementY * CONTROLS.aimSensitivity;

        CONTROLS.cameraYaw %= 360; // allow for turning around in circles, yaw goes from [0,360] in polar coord

        //this locks camera vertical movement so it behaves like a 'neck', since its FP
        if (CONTROLS.cameraPitch > 90) CONTROLS.cameraPitch = 90;
        if (CONTROLS.cameraPitch < -90) CONTROLS.cameraPitch = -90;
      }
      ...
      document.addEventListener('pointerlockerror', () => {
        ...
        //hacky stuff because pointer lock error makes everything behave weird
    ...
    function subscribeToMovementKeyPresses () {
    ...
    subscribeToMouseMovement();
    subscribeToMovementKeyPresses();
  }

  setupFPSInputControls();
  ...
  function render (timestamp) {
    ...
    function makeCameraMovable () {
      const { xAxis, yAxis, zAxis } = VectorMath.mat4.FPSLocalCameraCoordinates(
        CONTROLS.cameraEye,
        degToRad(CONTROLS.cameraPitch),
        degToRad(CONTROLS.cameraYaw)
      );

      const moveX = Number(pressedKey['d']) - Number(pressedKey['a']);
      const moveZ = Number(pressedKey['s']) - Number(pressedKey['w']);
      const moveY = Number(pressedKey[' ']) - Number(pressedKey['shift']);

      CONTROLS.cameraEye[0] += (xAxis[0] * moveX + yAxis[0] * moveY + zAxis[0] * moveZ) * CONTROLS.movementSensitivity;
      CONTROLS.cameraEye[1] += (xAxis[1] * moveX + yAxis[1] * moveY + zAxis[1] * moveZ) * CONTROLS.movementSensitivity;
      CONTROLS.cameraEye[2] += (xAxis[2] * moveX + yAxis[2] * moveY + zAxis[2] * moveZ) * CONTROLS.movementSensitivity;
    }

    function positionCameraLookingAtScene () {
      const eye = CONTROLS.cameraEye;
      const pitchRad = degToRad(CONTROLS.cameraPitch);
      const yawRad = degToRad(CONTROLS.cameraYaw);
      ...
      // FPSViewMatrix is different than lookAt
      const sceneViewMatrix = VectorMath.mat4.FPSViewMatrix(eye, pitchRad, yawRad);

      VectorMath.mat4.translate(sceneViewMatrix, CONTROLS.cameraPosition, sceneViewMatrix);
    ...
    const lightPosition = [(Math.sin(timestamp * 0.001) * 100), 100, (Math.cos(timestamp * 0.001) * 100)];

    function renderObjects (objectMemory: unknown, translation: number[], scale: number[], color: number[]) {
      const {
        uniformBuffer,
        uniformValues,
        bindGroup,
      ...
      const world = VectorMath.mat4.identity();

      VectorMath.mat4.translation(translation, world);
      ...
      VectorMath.mat4.scale(worldViewProjectionValue, scale, worldViewProjectionValue)
      ...
      lightDirectionValue.set(VectorMath.vec3.normalize(lightPosition));
    ...
    renderObjects(obj1, [0, 35, 0], [20, 20, 20], [1, 0, 0, 1])
    renderObjects(light, lightPosition, [5, 5, 5], [1, 1, 1, 0])
...
const VectorMath = {
  mat4: {
    ...
    FPSLocalCameraCoordinates (eye, pitch, yaw) {
    ...
    FPSViewMatrix (eye, pitch, yaw) {
      ...
      const viewMatrix = [
        ...
        -VectorMath.vec3.dot(xaxis, eye), -VectorMath.vec3.dot(yaxis, eye), -VectorMath.vec3.dot(zaxis, eye), 1
      ]
      return viewMatrix;
